...
public class NicoWnnGEngineEN implements WnnEngine {
     ...
     */
    public NicoWnnGEngineEN(String writableDictionaryName) {
        mConvResult = new ArrayList<WnnWord>();
        mCandTable = new HashMap<String, WnnWord>();
        mSearchKey = null;
        mOutputNum = 0;

        mDictionary = new NicoWnnGDictionaryImpl( 
        		"/data/data/net.gorry.android.input.nicownng/lib/libnicoWnnGEngDic.so",
        		writableDictionaryName);
        if (!mDictionary.isActive()) {
        	mDictionary = new NicoWnnGDictionaryImpl(
        			"/system/lib/libnicoWnnGEngDic.so",
        			writableDictionaryName);
        }
    ...
    public int addWords(WnnWord[] words, Runnable progress) {
        WnnDictionary dict = mDictionary;
        dict.setInUseState( true );
        dict.addWordToUserDictionary(words, progress);
        dict.setInUseState( false );
        return 0;
    }
